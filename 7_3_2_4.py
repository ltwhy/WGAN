...
discriminator_optim5 = torch.optim.Adam(discriminator5.parameters(), 2e-4, betas=(0.5, 0.999))
...
for epoch in range(num_epochs):
        ...
        for x, _ in train_loader:
            if x.size(0) == batch_size:
                ...
                if LOSS == 'wasserstein':
                    ...
                    -discriminator3(x_true).mean()+discriminator3(x_fake).mean()
                    -discriminator4(x_true).mean()+discriminator4(x_fake).mean()
                    -discriminator5(x_true).mean()+discriminator5(x_fake).mean()
                    error_true.backward()
                    ...
                    for i in range(disc_iters):
                        ...
                        - discriminator3(generator(z)).mean()
                        - discriminator4(generator(z)).mean()
                        - discriminator5(generator(z)).mean()
                        error_fake.backward()
        ...
        if ds =='CIFAR10':     
            ...
            for i, sample in enumerate(x_fake):
                ax = plt.subplot(gs[i])
                plt.axis('off')
                ax.set_xticklabels([])
                ax.set_yticklabels([])
